...
describe('createFlow', () => {
  it('should require a seed and return an object with a seed, no choice, and empty history, items, and operations', () => {
    ...
    const itemsArrayed = Array.isArray(flow.items)
    expect(itemsArrayed).toBe(true)
    const itemKeys = Object.keys(flow.items)
    expect(itemKeys.length).toBe(0)
    const operationsArrayed = Array.isArray(flow.operations)
    expect(operationsArrayed).toBe(true)
